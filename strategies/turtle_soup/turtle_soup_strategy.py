...
from rules.sma_filter_rule import SMAFilterRule, SMAFilterRuleConfig
...
class TurtleSoupStrategyConfig(StrategyConfig, frozen=True):
    ...
    liquidity_pool_upper_period_window: (
        int  # the upper period window | 3 on 1D TF means the last 3 daily bars highs inclusive
    )
    liquidity_pool_lower_period_window: (
        int  # the lower period window | 3 on 1D TF means the last 3 daily bars lows inclusive
    )
...
class TurtleSoupStrategy(RuleBasedStrategy):
    def __init__(self, config: TurtleSoupStrategyConfig):
        ...
        # initialize rules
        search_liquidity_pool_rule_config = SearchLiquidityPoolsRuleConfig(
        ...
        search_liquidity_pool_rule = SearchLiquidityPoolsRule(
            self.shared_state, self, search_liquidity_pool_rule_config
        )
        ...
        turtle_soup_rule_config = TurtleSoupMultiTFRuleConfig(
            levels_sources=[config.liquidity_pool_bar_type],
        ...
        expected_target_rule_config = ExpectedTargetRuleConfig(
            config.expected_target_bar_type, config.expected_target_left, config.expected_target_right
        )
        ...
        liquidity_pool_reuse_rule_config = LiquidityPoolReuseRuleConfig(
        ...
        entry_trading_rule = EntryTradingRule(
            ...
            self.config.fixed_lot,
            self.config.fixed_risk_percent,
        )
